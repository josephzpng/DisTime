...
def concat_pad_data_collator_multidatatype_time(features, max_item_length=None, pad_id=0):
    first = features[0]
    batch = {}

    data_type = [feat['data_type'] for feat in features]

    # get time_gt
    time_gt = [feat['time_gt'] for feat in features if 'time_gt' in feat]
    if not time_gt:
        time_gt = None

    frame_times = [feat['frame_times'] for feat in features if 'frame_times' in feat]
    if not frame_times:
        frame_times = None

    duration = [feat['duration'] for feat in features if 'duration' in feat]
    if not duration:
        duration = None
    ...
    for k, v in first.items():
        if k not in ('label', 'label_ids', 'pixel_values', 'image_flags', 'data_type','time_gt', 'frame_times', 'duration') and \
                v is not None and not isinstance(v, str):
            if isinstance(v, torch.Tensor):
                batch[k] = torch.stack([f[k] for f in features])
            elif isinstance(v, np.ndarray):
