...
from internvl.model.internvl_chat import (InternVisionConfig,
                                          ...
                                          InternVLChatConfig,
                                          InternVLChatModel,
                                          InternVLChatModelDisTime)
from internvl.patch import (concat_pad_data_collator, concat_pad_data_collator_multidatatype_time,
...
from internvl.train.dataset import (ConcatDataset, TCSLoader, read_frames_decord, read_frames_cv2, read_frames_cv2_v2,
...
class ModelArguments:
    ...
    use_pooling: bool = field(
        default=False,
        metadata={'help': 'Set to True to use pooling.'}
    )
    dec_reg: int = field(
        default=16,
        metadata={'help': 'Decoder reg_max'}
    )
    enc_sigma: float = field(
        default=1.0,
        metadata={'help': 'Encoder sigma'}
    )
    layer_num: int = field(
        default=3,
        metadata={'help': 'Encoder, Decoder layer num'}
    )
...
class LazySupervisedDataset(Dataset):
        ...
        if self.group_by_length:
            ...
            for data_item in self.raw_data:
                ...
                if 'length' in data_item:
                    ...
                    # BUG: Fixed by zepeng，value should be string
                    # conversations = '\n'.join([temp['value'] for temp in data_item['conversations']])
                    conversations = '\n'.join([str(temp['value']) for temp in data_item['conversations']])
    ...
    def video_get_item(self, data_item):
        ...
        image_list, frame_times, duration = read_frames_cv2_v2(video_path, max_num_frames=self.max_num_frame, min_num_frames=self.min_num_frame,
        ...
        time_gt = data_item['tgt'] if 'tgt' in data_item else None
        ...
        # Preprocess the conversations and generate the return dictionary
        num_image_tokens = [num_image_token] * num_patches
        ret = preprocess_function(self.template_name, [deepcopy(data_item['conversations'])],
                                  self.tokenizer, num_image_tokens, group_by_length=self.group_by_length,
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            if try_cnt > max_try:
                raise StopIteration("Max retries reached for loading data item.")
            try:
                ...
                if 'labels' in ret and torch.all(ret['labels'][1:] < 0):
                    print("All label[1:] are negative, reselect the data items")
...
def main():
    ...
    token_list = [IMG_START_TOKEN, IMG_END_TOKEN, IMG_CONTEXT_TOKEN,
                  ...
                  REF_END_TOKEN, BOX_START_TOKEN, BOX_END_TOKEN, TIME_CONTEXT_TOKEN, FRAME_TIME_REF_TOKEN]
    num_new_tokens = tokenizer.add_tokens(token_list, special_tokens=True)
    img_context_token_id = tokenizer.convert_tokens_to_ids(IMG_CONTEXT_TOKEN)
    tcs_loader = TCSLoader('~/petreloss.conf') if has_tcs_loader else None
    # tcs_loader = None
    # NOTE: add timestamp special token
    time_context_token_id = tokenizer.convert_tokens_to_ids(TIME_CONTEXT_TOKEN)

    frame_time_context_token_id = tokenizer.convert_tokens_to_ids(FRAME_TIME_REF_TOKEN)
    ...
    if model_args.model_name_or_path is not None:
        logger.info('Loading InternVLChatModelDisTime...')
        config = InternVLChatConfig.from_pretrained(model_args.model_name_or_path)
        config.use_pooling = model_args.use_pooling

        config.dec_reg = model_args.dec_reg
        config.enc_sigma = model_args.enc_sigma
        config.layer_num = model_args.layer_num
        ...
        if config.llm_config.model_type == 'internlm2':
            ...
            config.llm_config._attn_implementation = 'flash_attention_2'  # for LLaMA eager
        ...
        internvl_chat_config = InternVLChatConfig(
            ...
            use_pooling=model_args.use_pooling)
        internvl_chat_config.force_image_size = data_args.force_image_size
        logger.info('Building InternVLChatModelDisTime...')
        model = InternVLChatModelDisTime(internvl_chat_config, vision_model, llm)

    # NOTE: Init
    model.time_gen.init_weights()
    model.time_enc.init_weights()

    model.img_context_token_id = img_context_token_id
    # NOTE: add timestamp special token
    model.time_context_token_id = time_context_token_id

    model.frame_time_context_token_id = frame_time_context_token_id
    ...
    train_dataset = build_datasets(
        ...
        max_num_frame=data_args.max_num_frame, use_pooling=model_args.use_pooling)
    ...
    for m, p in model.named_parameters():
        if any(
            [
                x in m
                for x in ["output", "tok_embeddings", "time_enc", "time_gen"]
            ]
        ):
    ...
    if data_args.use_packed_ds:
        ...
        collator = concat_pad_data_collator_multidatatype_time
