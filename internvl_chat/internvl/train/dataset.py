...
def read_frames_cv2(video_path, max_num_frames, min_num_frames, sample = 'rand', fix_start = None, client = None, clip = None):
    cv2_vr = cv2.VideoCapture(video_path)
    # vlen = duration = int(cv2_vr.get(cv2.CAP_PROP_FRAME_COUNT)) - 5
    vlen = int(cv2_vr.get(cv2.CAP_PROP_FRAME_COUNT))

    fps = float(cv2_vr.get(cv2.CAP_PROP_FPS))
    ...
    for frame_idx in frame_indices:
        ...
        if not ret:
            raise ValueError(f'video error at {video_path},frame_idx {frame_idx}')
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        frame = Image.fromarray(frame)
        pil_image.append(frame)
    cv2_vr.release()
    return pil_image, duration


def read_frames_cv2_v2(video_path, max_num_frames, min_num_frames, sample = 'rand', fix_start = None, client = None, clip = None):
    cv2_vr = cv2.VideoCapture(video_path)
    vlen = int(cv2_vr.get(cv2.CAP_PROP_FRAME_COUNT))
    fps = float(cv2_vr.get(cv2.CAP_PROP_FPS))
    t_num_frames = np.random.randint(min_num_frames, max_num_frames + 1)
    duration = round(vlen / fps, 2)
    if duration < 1:
        raise ValueError(f'video: {video_path}, duration < 1')
    ...
    for frame_idx in frame_indices:
        ...
        frame = Image.fromarray(frame)
        pil_image.append(frame)
        frame_times.append(round(frame_idx / fps, 2))

    cv2_vr.release()
    return pil_image, frame_times, duration
...
class TCSLoader(object):
    ...
    def __call__(self, fn, image_type='image', max_num_frames=-1, min_num_frames=4, sample='rand', clip=None, use_scene_det=False):
        is_feat = False
        if image_type == 'image':
            ...
            if fn.endswith('.gif'):
                frames, duration = read_frames_gif(fn, num_frames=max_num_frames, min_num_frames=min_num_frames,
                                         client=self.client, sample=sample)
            ...
            #                                 client=self.client, sample=sample)
                ...
                frames, duration = read_frames_cv2(fn, max_num_frames=max_num_frames, min_num_frames=min_num_frames,
                                            client=self.client, sample=sample, clip=clip, use_scene_det=use_scene_det)
            return frames, duration
