...
def collate_fn(batches, tokenizer):
    ...
    return pixel_values, questions, answers, num_patches_lists, task_types, duration, frame_times
...
class MVBenchDataset(torch.utils.data.Dataset):
    ...
    def read_gif(self, video_path, bound=None, fps=25):
        ...
        max_frame = len(gif) - 1
        duration = round(max_frame / fps, 2)
        images_group = list()
        frame_times = list()
        frame_indices = self.get_index(bound, fps, max_frame, first_idx=0)
    ...
    def __getitem__(self, idx):
        ...
        image_list, frame_times, duration = decord_method(video_path, bound)
        ...
        question = special_tokens + self.prompt + '\n' + question + self.question_prompt
...
def evaluate_chat_model():
    ...
    outputs = []
    for _, (pixel_values, questions, answers, num_patches_lists, task_types, duration, frame_times) in tqdm(enumerate(dataloader), total=len(dataloader)):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out-dir', type=str, default='results/MVBench/8B')
