...
def collate_fn(batches):
    ...
    target = [_['target'] for _ in batches]
    frame_times = [_['frame_times'] for _ in batches]

    return pixel_values, questions, query, video_id, num_patches_lists, duration, target, frame_times

class QVHDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target = self.json_dict[idx]['target']
...
def evaluate_chat_model():
    ...
    json_file = 'data_example/qvhighlights/val.json'
    video_root = 'data/qvhighlights/videos' # qvhighlights video path

    vid_dataset = QVHDataset(
        json_file=json_file, 
    ...
    results = []
    for _, (pixel_values, questions, query, video_id, num_patches_lists, duration, target, frame_times) in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        res['qid'] = video_id[0]
        res['query'] = query[0]
        res['prediction'] = str(time_stamp_str)
        res['target'] = str(target[0])
        res['duration'] = duration[0]
        
        results.append(res)

    _ = save_result_merged(results, args.out_dir, 'results')
...
def save_result_merged(result, result_dir, filename, remove_duplicate=""):
    import json
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasets', type=str, default='qvh')
    parser.add_argument('--batch-size', type=int, default=1)
    parser.add_argument('--num-workers', type=int, default=1)
    parser.add_argument('--num-beams', type=int, default=1)
    parser.add_argument('--temperature', type=float, default=0.0)
    parser.add_argument('--out-dir', type=str, default='results/QVHighlight/8B')
