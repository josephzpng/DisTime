...
#         # print(pred)
#         # print( groundtruths[idx]['timestamp'])
...
def get_miou(predictions, groundtruths):
    ...
    for idx in groundtruths.keys():
        if idx not in predictions:
            ...
            continue
        pred = predictions[idx]["timestamp"][0]  # top1
        iou = get_iou(pred, groundtruths[idx]['timestamp'])
        ious.append(iou)
        iou_dict[idx] = iou
...
def eval_result(result_file, gt_file, detail=False):
    ...
    for iou, max_proposal_num in list(itertools.product([0.7, 0.5, 0.3], [1])):
        if detail:
...
if __name__ == '__main__':
    result_file = 'internvl_chat/results/ANet-Caption/8B/results.json'
    gt_file = 'internvl_chat/data_example/anet/val_2.json'
    print(result_file)
    eval_result(result_file, gt_file)