...
def collate_fn(batches):
    ...
    video_id_uni = [_['video_id_uni'] for _ in batches]
    anno_id = [_['anno_id'] for _ in batches]
    ...
    return pixel_values, questions, query, video_id, video_id_uni, anno_id, answers, num_patches_lists, duration, frame_times
...
class ANetDataset(torch.utils.data.Dataset):
    def __init__(self, json_file, video_file, num_segments=16, input_size=224,
        ...
        with open(json_file, 'r') as f:
            ...
            for item in data:
                ...
                queries = data[item]['sentences']

                for query_id in range(len(queries)):
                    item_dict = dict(
                        video_id=item,
                        video_id_uni=item + '-' + str(query_id),
                        anno_id=query_id,
                        query=queries[query_id],
                        video=self.video_dict[item],
                        gt_timestamps=data[item]['timestamps'][query_id]
                    )
    ...
    def __getitem__(self, idx):
        ...
        video_id_uni = self.json_dict[idx]['video_id_uni']
        anno_id = self.json_dict[idx]['anno_id']
        ...
        answer = self.json_dict[idx]['gt_timestamps']
...
def evaluate_chat_model():
    ...
    res = {}
    result = {}
    for _, (pixel_values, questions, query, video_id, video_id_uni, anno_id, answers, num_patches_lists, duration, frame_times) in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        time_stamp_str = postprocess_pred_time(time, duration[0], model.reg_max) if len(time) !=0 else [[-1, -1]]
            
        res[video_id_uni[0]] = dict(
                video_id=video_id[0],
                anno_id=anno_id[0],
    ...
    _ = save_result_merged(result, args.out_dir, 'results')
...
def save_result_merged(result, result_dir, filename):
    ...
    if is_main_process():
        ...
        for rank in range(get_world_size()):
            ...
            res = json.load(open(result_file, "r"))['results']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasets', type=str, default='anet')
    parser.add_argument('--batch-size', type=int, default=1)
    parser.add_argument('--num-workers', type=int, default=1)
    parser.add_argument('--num-beams', type=int, default=1)
    parser.add_argument('--temperature', type=float, default=0.0)
    parser.add_argument('--out-dir', type=str, default='results/ANet-Caption/8B')
