...
if [ ${DATASET} == "charades" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/charades-sta/evaluate_charades.py --checkpoint ${CHECKPOINT} --datasets charades "${ARGS[@]:2}"
fi

if [ ${DATASET} == "anet" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/anet/evaluate_anet.py --checkpoint ${CHECKPOINT} --datasets anet "${ARGS[@]:2}"
fi
...
if [ ${DATASET} == "qvh" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/qvhighlight/evaluate_qvhighlight.py --checkpoint ${CHECKPOINT} --datasets qvh "${ARGS[@]:2}"
fi

if [ ${DATASET} == "anet_dvc" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/anet_dvc/evaluate_anet_dvc.py --checkpoint ${CHECKPOINT} --datasets anet_dvc "${ARGS[@]:2}"
fi

if [ ${DATASET} == "youcook2_dvc" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/youcook2_dvc/evaluate_youcook2_dvc.py --checkpoint ${CHECKPOINT} --datasets youcook2_dvc "${ARGS[@]:2}"
fi
...
if [ ${DATASET} == "nextgqa" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/nextgqa/evaluate_nextgqa.py --checkpoint ${CHECKPOINT} --datasets nextgqa --stage ${STAGE} "${ARGS[@]:3}"
fi
...
if [ ${DATASET} == "mvbench" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/mvbench/evaluate_mvbench_distime.py --checkpoint ${CHECKPOINT} --datasets mvbench "${ARGS[@]:2}"
fi
...
if [ ${DATASET} == "longvideobench" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/longvideobench/evaluate_longvideobench_distime.py --checkpoint ${CHECKPOINT} --datasets longvideobench "${ARGS[@]:2}"
fi